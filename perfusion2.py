...
F = -K1 * dot(grad(p1), grad(q1))*dx -K2 * dot(grad(p2), grad(q2))*dx -K3 * dot(grad(p3), grad(q3))*dx  + dot(beta12*(p1-p2),q1)*dx + dot(beta12*(p2-p1),q2)*dx + dot(beta23*(p2-p3),q2)*dx + dot(beta23*(p3-p2),q3)*dx - dot(-Constant(0.1)*(p3-Constant(3.0)),q3)*dx
